...
if __name__ == '__main__':
 ...
 #      levels=[-0.7 -0.5, -0.3 -0.1, 0.1, 0.3, 0.5, 0.7]
       ...
       map = Basemap(projection='cyl',llcrnrlat=30,urcrnrlat=80,llcrnrlon=-20,urcrnrlon=120,resolution='c')
       map.drawcoastlines()
       map.drawparallels(np.arange(30,80,10),labels=[1,0,0,0])
       map.drawmeridians(np.arange(-20,120,20),labels=[0,0,0,1])
       ...
       cbar=map.colorbar(im,"bottom", size="5%", pad="15%")
    #   plt.show()
       fig.savefig(datapath_out+paramname+'_ensmean_eurasia_'+exp2+'-'+exp1+'_'+season+'.png')
