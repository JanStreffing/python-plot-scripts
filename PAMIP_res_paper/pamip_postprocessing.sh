...
# t2m hPa polar plots 
if [[ " $todo " =~ .*\ "t2m"\ .* ]]; then
    python nh_stereo_diff.py 11 16 T159 T2M T2M $in_mistral 1 $out_mistral colorbar_TR_70 -7,-5,-3,-1,-0.5,-0.3,-0.1,0.1,0.3,0.5,1,3,5,7
    #python nh_stereo_diff_ngl_append.py 11 16 T159 T2M T2M $in_mistral 1 $out_mistral 
    #python nh_stereo_diff_ngl_append_basemap.py 11 16 T159 T2M T2M $in_mistral 1 $out_mistral 
fi
