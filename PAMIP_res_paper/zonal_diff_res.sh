...
for todo in force #Z #pch #sin_diff T_for_doug U_for_doug sin_diff T U UP iso_diff
do
        if [[ "$todo" == "force" ]]; then
	...
	# Z zonal plots 
	if [[ "$todo" == "Z" ]]; then
		python zonal_diff_res.py 11 16 T159 Z Z $in 9.81 $out true  -15,-10,-6,-3,-1,-0.3,0.3,1,3,6,10,15 colorbar_TR_70 14
		#-2,-1.5,-1,-0.5,-0.2,0.2,0.5,1,1.5,2
	fi

	# T zonal plots 
	if [[ "$todo" == "T" ]]; then
		python zonal_diff_res.py 11 16 T159 T T $in 1 $out true  -7,-5,-3,-1,-0.5,-0.3,-0.1,0.1,0.3,0.5,1,3,5,7 colorbar_TR_70 14
		#-2,-1.5,-1,-0.5,-0.2,0.2,0.5,1,1.5,2
	fi

	# U zonal plots
	if [[ "$todo" == "U" ]]; then
		python zonal_diff_res.py 11 16 T159 U U $in 1 $out true -1.5,-1.3,-1.1,-0.9,-0.7,-0.5,-0.3,-0.1,0.1,0.3,0.5,0.7,0.9,1.1,1.3,1.5 colorbar_TR_15 14
