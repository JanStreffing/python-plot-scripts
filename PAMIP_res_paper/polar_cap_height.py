...
def bootstrap(xyobs, data1, data2):
        ...
        for lat in tqdm(range(0,tstarobs.shape[1])):
                for lon in range(0,tstarobs.shape[2]):
                        p1 = tstar[:,0,plev,time][tstar[:,0,plev,time] >= tstarobs[0,plev,time]].shape[0]/B
                        p2 = tstar[:,0,plev,time][tstar[:,0,plev,time] <= -tstarobs[0,plev,time]].shape[0]/B
                        p = p1 + p2
                        pvalue[lat,lon] = p
        return pvalue
...
# --- Calculating mean
		mean1 = np.mean(np.asarray(data1),axis=0)
		mean2 = np.mean(np.asarray(data2),axis=0)
		xyobs = np.concatenate([mean1,mean2])
...
# --- Plot absolute data
		...
		print(len(plev))
		print(mean1.shape)
		im=plt.contourf(time, plev, mean2-mean1, levels=mapticks, cmap=cmap_TR, extend='both',zorder=1)
		...
		fig.savefig(outpath+'pch_'+exp2+'_'+exp1+'_diff.png', dpi=150)
